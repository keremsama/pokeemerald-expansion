...
static u16 BagList_GetItemId(u8 pocketId, u32 pos);
...
static void BagMenu_GetEVStatName(u8 effectType, u8 *dest);
...
static void BagMenu_LoadHeldItemIconGfx(u16);
...
static void MergeSort(struct BagPocket *pocket, s32 (*comparator)(u8, struct ItemSlot, struct ItemSlot));
static s32 CompareItemsAlphabetically(u8 pocketId, struct ItemSlot item1, struct ItemSlot item2);
static s32 CompareItemsByMost(u8 pocketId, struct ItemSlot item1, struct ItemSlot item2);
static s32 CompareItemsByType(u8 pocketId, struct ItemSlot item1, struct ItemSlot item2);
static s32 CompareItemsByIndex(u8 pocketId, struct ItemSlot item1, struct ItemSlot item2);
...
static const u32 sBagScreen_Gfx[]               = INCBIN_U32("graphics/bag/swsh/tiles.4bpp.lz");
static const u16 sBagScreen_Pal[]               = INCBIN_U16("graphics/bag/swsh/tiles.gbapal");
static const u32 sBagScreen_BG2TileMap[]        = INCBIN_U32("graphics/bag/swsh/bg2.bin.lz");
static const u32 sBagScreen_BG3TileMap[]        = INCBIN_U32("graphics/bag/swsh/bg3.bin.lz");
static const u32 sCursor_Gfx[]                  = INCBIN_U32("graphics/bag/swsh/cursor.4bpp.lz");
static const u32 sHoverSlot_Gfx[]               = INCBIN_U32("graphics/bag/swsh/hover_slot.4bpp.lz");
static const u32 sScrollThumb_Gfx[]             = INCBIN_U32("graphics/bag/swsh/scroll_thumb.4bpp.lz");
static const u32 sCategoryIcons_Gfx[]           = INCBIN_U32("graphics/bag/swsh/category_icons.4bpp.lz");
static const u32 sPocketScrollArrows_Gfx[]      = INCBIN_U32("graphics/bag/swsh/pocket_scroll_arrows.4bpp.lz");
static const u32 sSwapCursor_Gfx[]              = INCBIN_U32("graphics/bag/swsh/swap_cursor.4bpp.lz");
static const u32 sFrameMoney_Gfx[]              = INCBIN_U32("graphics/bag/swsh/frame_money.4bpp.lz");
static const u32 sFramePriceQuantity_Gfx[]      = INCBIN_U32("graphics/bag/swsh/frame_price_quantity.4bpp.lz");
...
static const u32 sMoveTypeIcons_Gfx[]           = INCBIN_U32("graphics/bag/swsh/move_types.4bpp.lz");
...
#endif
#if SWSH_ITEM_MENU_IN_BAG_USE
static const u32 sStatusIcons_Gfx[]             = INCBIN_U32("graphics/bag/swsh/status_icons.4bpp.lz");
#if SWSH_ITEM_MENU_IN_BATTLE_USE
static const u32 sMultiBattleSwapPrompt_Gfx[]   = INCBIN_U32("graphics/bag/swsh/prompt_swap.4bpp.lz");
#endif
#endif
...
extern void DeleteMove(struct Pokemon *mon, u16 move);
...
static bool8 LoadBagMenu_Graphics(void)
{
    ...
    case 1:
        ...
        {
            LZDecompressWram(sBagScreen_BG2TileMap, gBagMenu->mainTilemapBuffer);
            LZDecompressWram(sBagScreen_BG3TileMap, gBagMenu->scrollingBgTilemapBuffer);
            gBagMenu->graphicsLoadState++;
        }
        break;
    case 2:
    ...
    case 10:
        ...
        LZDecompressWram(sMoveTypeIcons_Gfx, sMoveTypeIconsCache);
        gBagMenu->graphicsLoadState++;
        break;
    case 11:
...
static u16 BagList_GetItemId(u8 pocketId, u32 pos)
{
    if (pocketId >= POCKETS_COUNT || pos >= gBagPockets[pocketId].capacity)
        return ITEM_NONE;

    return gBagPockets[pocketId].itemSlots[pos].itemId;
}

static struct ItemSlot BagList_GetSlot(u8 pocketId, u32 pos)
{
    if (pocketId >= POCKETS_COUNT || pos >= gBagPockets[pocketId].capacity)
        return (struct ItemSlot){ ITEM_NONE, 0 };

    return (struct ItemSlot){
        .itemId = gBagPockets[pocketId].itemSlots[pos].itemId,
        .quantity = BagGetQuantityByPocketPosition(pocketId + 1, pos),
    };
}

static void BagList_MoveSlot(u8 pocketId, u32 from, u32 to)
{
    MoveItemSlotInList(gBagPockets[pocketId].itemSlots, from, to);
}
#endif
...
static u16 GetItemTMHMIndex(u16 itemId)
{
    ...
    if (itemId >= ITEM_HM01 && itemId < ITEM_HM01 + NUM_HIDDEN_MACHINES)
        return itemId - ITEM_HM01 + NUM_TECHNICAL_MACHINES + 1;
    return 0;
}
...
static void BagMenu_MoveCursorCallback(s32 itemIndex, bool8 onInit, struct ListMenu *list)
{
...
#if SWSH_ITEM_MENU_IN_BAG_USE
    if (gBagPosition.pocket == TMHM_POCKET && gBagPosition.location != ITEMMENULOCATION_PARTY)
    ...
    {
        if (gBagPosition.pocket == TMHM_POCKET && gBagMenu->moveInfoMode == 1)
            UpdateMoveBattleInfo(itemIndex);
#if SWSH_ITEM_MENU_CONTEST_INFO
        else if (gBagPosition.pocket == TMHM_POCKET && gBagMenu->moveInfoMode == 2)
            PrintContestDescription(itemIndex);
        else if (gBagPosition.pocket == TMHM_POCKET && gBagMenu->moveInfoMode == 3)
            UpdateMoveContestInfo(itemIndex);
#endif
#if SWSH_ITEM_MENU_BERRY_STAT
        else if (gBagPosition.pocket == BERRIES_POCKET && gBagMenu->berryInfoMode == 1)
            UpdateBerryInfo(itemIndex);
#if SWSH_ITEM_MENU_BERRY_TAG
        else if (gBagPosition.pocket == BERRIES_POCKET && gBagMenu->berryInfoMode == 2)
...
static void BagMenu_ItemPrintCallback(u8 windowId, u32 itemIndex, u8 y)
{
    if (itemIndex != LIST_CANCEL)
    {
        ...
        // Draw HM icon
        if (gBagPosition.pocket == TMHM_POCKET && GetItemTMHMIndex(itemSlot.itemId) > NUM_TECHNICAL_MACHINES)
            BlitBitmapToWindow(windowId, sBagMenuHMIcon_Gfx, 8, y, 16, 16);

        bool8 isHovered = ((s32)itemIndex == gBagMenu->hoveredItemIndex);

        if (gBagPosition.pocket != KEYITEMS_POCKET && GetItemImportance(itemSlot.itemId) == FALSE)
        {
            // Print item quantity
...
void UpdatePocketItemList(u8 pocketId)
{
    ...
    for (u32 i = 0; i < pocket->capacity && pocket->itemSlots[i].itemId; i++)
        gBagMenu->numItemStacks[pocketId]++;
...
static bool8 BagMenu_TryWraparoundScroll(u8 listTaskId, u16 *scrollPos, u16 *cursorPos)
{
    ...
    u16 realTotal = total - (gBagMenu->hideCloseBagText ? 0 : 1);
    u16 absPos = list->scrollOffset + list->selectedRow;
    ...
    if (JOY_NEW(DPAD_UP) && list->scrollOffset == 0 && list->selectedRow == 0)
    {
        ...
        if (realTotal > maxShowed)
        {
            list->scrollOffset = realTotal - maxShowed;
            list->selectedRow = maxShowed - 1;
        }
        else
        {
            list->scrollOffset = 0;
            list->selectedRow = realTotal - 1;
        }
    }
    else if (JOY_NEW(DPAD_DOWN) && absPos == realTotal - 1)
    {
        list->scrollOffset = 0;
        list->selectedRow = 0;
    }
    ...
    *cursorPos = list->selectedRow;
    absPos = list->scrollOffset + list->selectedRow;
    list->template.moveCursorFunc(list->template.items[absPos].id, FALSE, list);
    return TRUE;
}
...
static void Task_BagMenu_HandleInput(u8 taskId)
{
    ...
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        default:
            ...
            {
                bool8 pocketEmpty = gBagMenu->numItemStacks[gBagPosition.pocket] == (u8)(!gBagMenu->hideCloseBagText);
                if (!pocketEmpty && gBagPosition.pocket == TMHM_POCKET)
                {
                    PlaySE(SE_SELECT);
                    SwitchMoveInfoMode(gBagMenu->hoveredItemIndex);
                }
#if SWSH_ITEM_MENU_BERRY_STAT
                else if (!pocketEmpty && gBagPosition.pocket == BERRIES_POCKET)
                {
                    PlaySE(SE_SELECT);
...
static void SwitchBagPocket(u8 taskId, s16 deltaBagPocketId, bool16 skipEraseList)
{
    ...
    if (!skipEraseList)
    {
        if (gBagPosition.pocket == TMHM_POCKET && gBagMenu->moveInfoMode)
        {
...
#if SWSH_ITEM_MENU_BERRY_STAT
        if (gBagPosition.pocket == BERRIES_POCKET && gBagMenu->berryInfoMode)
        {
...
#endif
        {
            bool8 srcHasInfoPrompt = (gBagPosition.pocket == TMHM_POCKET);
            bool8 destHasInfoPrompt = (newPocket == TMHM_POCKET);
#if SWSH_ITEM_MENU_BERRY_STAT
            srcHasInfoPrompt |= (gBagPosition.pocket == BERRIES_POCKET);
            destHasInfoPrompt |= (newPocket == BERRIES_POCKET);
#endif
...
{
    // Safety check
    u8 type = GetItemType(gSpecialVar_ItemId);
    if (!GetItemBattleUsage(gSpecialVar_ItemId))
        return;
...
static void PrintContestDescription(s32 itemIndex)
{
    ...
    if (itemIndex != LIST_CANCEL)
    {
        u16 move = ItemIdToBattleMoveId(BagList_GetItemId(gBagPosition.pocket, itemIndex));
...
static u8 BagMenu_GetPocketIdByPocket(const struct BagPocket *pocket)
{
    u8 i;

    for (i = 0; i < POCKETS_COUNT; i++)
    {
        if (pocket == &gBagPockets[i])
            return i;
    }
...
static inline __attribute__((always_inline)) void Merge(struct BagPocket *pocket, u32 iLeft, u32 iRight, u32 iEnd, struct ItemSlot *dummySlots, s32 (*comparator)(u8, struct ItemSlot, struct ItemSlot))
{
    struct ItemSlot item_i, item_j;
    u32 i = iLeft, j = iRight;
    u8 pocketId = BagMenu_GetPocketIdByPocket(pocket);

    for (u32 k = iLeft; k < iEnd; k++)
    {
        item_i = pocket->itemSlots[i];
        item_j = (j < iEnd) ? pocket->itemSlots[j] : (struct ItemSlot){0};
        if (i < iRight && (j >= iEnd || comparator(pocketId, item_i, item_j) < 0))
        {
            dummySlots[k] = item_i;
            i++;
        }
...
static void MergeSort(struct BagPocket *pocket, s32 (*comparator)(u8, struct ItemSlot, struct ItemSlot))
{
    ...
    for (u32 width = 1; width < usedCapacity; width *= 2)
    {
        ...
        for (u32 j = 0; j < usedCapacity; j++)
            pocket->itemSlots[j] = dummySlots[j];
    }
...
static s32 CompareItemsAlphabetically(u8 pocketId, struct ItemSlot item1, struct ItemSlot item2)
{
    ...
    {
        name1 = GetMoveName(ItemIdToBattleMoveId(item1.itemId));
        name2 = GetMoveName(ItemIdToBattleMoveId(item2.itemId));
    }
    else
    {
...
static s32 CompareItemsByType(u8 pocketId, struct ItemSlot item1, struct ItemSlot item2)
{
    ...
    u8 type1 = GetItemType(item1.itemId);
    u8 type2 = GetItemType(item2.itemId);
...
static s32 CompareItemsByIndex(u8 pocketId, struct ItemSlot item1, struct ItemSlot item2)
{
    ...
    case TMHM_POCKET:
        ...
        break;
    case BERRIES_POCKET: // To do - requires #7305
        index1 = item1.itemId;
        index2 = item2.itemId;
        break;
    default:
...
#if SWSH_ITEM_MENU_PARTY_PANEL && !SWSH_ITEM_MENU_IN_BAG_USE

// ============================================================
// Party Panel display-only backport
// ============================================================
...
{
    return slot < PARTY_SIZE
        && GetMonData(&gPlayerParty[slot], MON_DATA_SPECIES_OR_EGG) != SPECIES_NONE;
}
        ...
        for (row = 0; row < PARTY_PANEL_SLOT_HEIGHT; row++)
        {
            ...
            for (col = 0; col < PARTY_PANEL_SLOT_WIDTH; col++)
            {
                buf[panelRow * 32 + (PARTY_PANEL_START_COL + col)]
                    = (sPartySlots_Tilemap[binRow * PARTY_PANEL_SLOT_WIDTH + col] & 0x0FFF)
                    | (PARTY_SLOT_NORMAL_PAL << 12);
            }
        }
    }
}

static void BagMenu_CreatePartyIcon(u8 slot)
{
    ...
    spriteId = CreateMonIcon(species, SpriteCB_MonIcon, 30, 24 * slot + 16, 6, GetMonData(mon, MON_DATA_PERSONALITY));
    if (spriteId == MAX_SPRITES)
        return;
...
#endif // SWSH_ITEM_MENU_PARTY_PANEL && !SWSH_ITEM_MENU_IN_BAG_USE
...
static void BagMenu_UpdateTMHMPartyBlend(s32 itemIndex)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        else
        {
            ...
            u16 itemId = BagList_GetItemId(gBagPosition.pocket, itemIndex);
...
static void BagMenu_GetMedicineEffectMessage(u16 item, u32 statusCured)
{
...
static u16 BagMenu_GetMonEV(struct Pokemon *mon, u8 effectType)
{
    switch (effectType)
    {
    case ITEM_EFFECT_HP_EV:
...
static void Task_BagMenu_MoveLearnAfterForget(u8 taskId)
{
    ...
    {
        struct Pokemon *mon = &gParties[B_TRAINER_PLAYER][sBagItemUseState->slot];
        u16 oldMove = GetMonData(mon, MON_DATA_MOVE1 + moveSlot);
...
#if SWSH_ITEM_MENU_IN_BATTLE_USE
static bool8 BagMenu_IsItemFlute(u16 item)
{
...
void GoToBattlePyramidBagMenu(u8 location, MainCallback exitCallback)
{
    ...
    GoToBagMenu(itemMenuLocation, ITEMS_POCKET, exitCallback);
    if (gBagMenu != NULL)
    {
...
void InitBattlePyramidBagCursorPosition(void)
{
    ...
    gPyramidBagMenuState.scrollPosition = 0;
    gBagPosition.cursorPosition[ITEMS_POCKET] = 0;
    gBagPosition.scrollPosition[ITEMS_POCKET] = 0;
}
