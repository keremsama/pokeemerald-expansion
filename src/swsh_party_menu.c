...
#define CHECK_EVO 0
#define DO_EVO 1
#ifndef FLDEFF_USE_ROCK_CLIMB
#define FLDEFF_USE_ROCK_CLIMB 0
#endif
#ifndef TEXT_SPEED_INSTANT
...
#define P_FLAG_EGG_MOVES 0
#define P_TM_MOVES_RELEARNER FALSE
#define P_FLAG_TUTOR_MOVES 0
#define P_ASK_MOVE_CONFIRMATION TRUE
...
#define RELEARN_MODE_SCRIPT 0
#define RELEARN_MODE_PARTY_MENU 1
#define PYRAMID_LOCATION_NONE 0
#define CurrentBattlePyramidLocation() PYRAMID_LOCATION_NONE
#define FIRST_BATTLE_MSG_FLAG_PARTY_MENU 0
#define BtlCtrl_OakOldMan_TestState2Flag(flag) FALSE
#define BtlCtrl_OakOldMan_SetState2Flag(flag)
#define IsOnPlayerSide(battler) (GetBattlerSide(battler) == B_SIDE_PLAYER)

#define FOLLOWER_NPC_FLAG_CAN_SURF        0
#define FOLLOWER_NPC_FLAG_CAN_LEAVE_ROUTE 0
#define FOLLOWER_NPC_FLAG_CAN_WATERFALL   0
#define FOLLOWER_NPC_FLAG_CAN_DIVE        0
#define PlayerHasFollowerNPC()            FALSE
#define GetFollowerNPCObjectId()          0
#define FollowerNPCWalkIntoPlayerForLeaveMap()
#define FollowerNPCHideForLeaveMap(follower)
#define CheckFollowerNPCFlag(flag)        TRUE
#define FollowerNPCIsBattlePartner()      FALSE
#define MetatileBehavior_IsRockClimbable(metatile) FALSE
#define CreateMonIconIsEgg(species, callback, x, y, priority, pid, isEgg) CreateMonIcon(species, callback, x, y, priority, pid)
#define gText_MoveItemWhere gText_MoveToWhere
#define gText_CannotSendMonToBoxPartner gText_CannotSendMonToBoxHM

static const u8 sText_XsYAnd_SwSh[] = _("{STR_VAR_1}'s {STR_VAR_2} and\n");
static const u8 sText_XsYWereSwapped_SwSh[] = _("{STR_VAR_1}'s {STR_VAR_2}\nwere swapped!{PAUSE_UNTIL_PRESS}");
#define gText_XsYAnd sText_XsYAnd_SwSh
#define gText_XsYWereSwapped sText_XsYWereSwapped_SwSh

static inline u16 GetEvolutionTargetSpeciesCompat(struct Pokemon *mon, enum EvolutionMode mode, u16 evolutionItem, struct Pokemon *tradePartner)
{
    return GetEvolutionTargetSpecies(mon, mode, evolutionItem, tradePartner);
}
#define GetEvolutionTargetSpecies(mon, mode, item, tradePartner, ...) GetEvolutionTargetSpeciesCompat(mon, mode, item, tradePartner)
...
static inline bool32 CanBoxMonRelearnAnyMove(struct BoxPokemon *boxMon)
{
    return FALSE;
}

static inline bool32 HasRelearnerEggMoves(struct Pokemon *mon)
{
    return FALSE;
}

static inline bool32 HasRelearnerTMMoves(struct Pokemon *mon)
{
    return FALSE;
}

static inline bool32 HasRelearnerTutorMoves(struct Pokemon *mon)
{
    return FALSE;
}

static inline bool32 HasRelearnerLevelUpMoves(struct Pokemon *mon)
{
    return FALSE;
}
...
static EWRAM_DATA bool8 sShadowAnimDelayTaskActive = FALSE;
...
static void CB2_ShowPokemonSummaryScreen(void)
{
    ...
    if (SWSH_SUMMARY_SCREEN)
        ShowPokemonSummaryScreen_SwSh(mode, gPlayerParty, gPartyMenu.slotId, gPlayerPartyCount - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
    else if (BW_SUMMARY_SCREEN)
        ShowPokemonSummaryScreen_BW(mode, gPlayerParty, gPartyMenu.slotId, gPlayerPartyCount - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
    else
        ShowPokemonSummaryScreen(mode, gPlayerParty, gPartyMenu.slotId, gPlayerPartyCount - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
}
...
static u8 LoadMonGfxAndSprite(struct Pokemon *mon, s16 *state, bool32 isShadow)
{
    ...
    case 0:
        ...
        return 0xFF;
    case 1:
        LoadCompressedSpritePaletteWithTag(GetMonSpritePalFromSpeciesAndPersonality(species, isShiny, pid), species);
