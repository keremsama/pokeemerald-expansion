...
{
    return GetPocketByItemId(itemId) == POCKET_TM_HM;
}
...
static const u16 sRandomizerMegaStonePool[] =
{
    ITEM_VENUSAURITE,
    ITEM_CHARIZARDITE_X,
    ITEM_CHARIZARDITE_Y,
...
u16 RandomizeFoundItem(u16 itemId, u8 mapNum, u8 mapGroup, u8 localId)
{
    ...
    if (IsMegaStone(itemId))
        return sRandomizerMegaStonePool[RandomizerNextRange(&state, ARRAY_COUNT(sRandomizerMegaStonePool))];
