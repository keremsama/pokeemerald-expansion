...
static void HideGradeIcons(void);
...
static bool8 LoadGraphics(void)
{
    ...
    case 24:
    {
        ...
        if (sMonSummaryScreen->mode != SUMMARY_MODE_SELECT_MOVE)
            inputTaskId = CreateTask(Task_HandleInput, 0);
        else
            inputTaskId = CreateTask(Task_SetHandleReplaceMoveInput, 0);
        gTasks[inputTaskId].data[3] = DEFAULT_SKILLS_STATE;
        gMain.state++;
        break;
    }
    case 25:
...
static bool8 DecompressGraphics(void)
{
    ...
    case 1:
        ...
        {
            LZDecompressWram(sSummaryPage_Info_Tilemap, sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_INFO]);
            sMonSummaryScreen->switchCounter++;
        }
        break;
    case 2:
        LZDecompressWram(sSummaryPage_Skills_Tilemap, sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_SKILLS]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 3:
        LZDecompressWram(sSummaryPage_BattleMoves_Tilemap, sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_BATTLE_MOVES]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 4:
        LZDecompressWram(sSummaryEffect_Battle_Tilemap, sMonSummaryScreen->bg1TilemapBuffers[PSS_EFFECT_BATTLE]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 5:
        LZDecompressWram(sSummaryPage_ScrollBG_Tilemap, sMonSummaryScreen->bg3TilemapBuffers);
        sMonSummaryScreen->switchCounter++;
        break;
    case 6:
    ...
    case 15:
        ...
        break;
    case 16:
        if (SWSH_SUMMARY_IV_EV_DISPLAY == SWSH_IV_EV_GRADED)
...
static void Task_HandleInput(u8 taskId)
{
    ...
    u8 defaultSkillsState = DEFAULT_SKILLS_STATE;
    ...
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        else if (JOY_NEW(A_BUTTON))
        {
            ...
            {
                if (SWSH_SUMMARY_IV_EV_DISPLAY != SWSH_IV_EV_HIDDEN)
                {
...
static void Task_ChangeSummaryMon(u8 taskId)
{
    ...
    case 12:
        ...
        {
            DrawNextSkillsButtonPrompt(DEFAULT_SKILLS_STATE);
        }
        break;
    case 13:
...
static void PrintMonAbilityName(void)
{
    u16 ability = GetAbilityBySpecies(sMonSummaryScreen->summary.species, sMonSummaryScreen->summary.abilityNum, TRUE);
...
static void BufferAndPrintStats_HandleState(u8 mode)
{
    ...
    if (SWSH_SUMMARY_IV_EV_DISPLAY == SWSH_IV_EV_GRADED && mode != SKILL_STATE_STATS)
    {
        ShowGradeIcons(mode);
...
static void ShowGradeIcons(u8 mode)
{
    ...
    {
        u32 gradeIndex = i - SPRITE_ARR_ID_HP_GRADE;

        if (sMonSummaryScreen->spriteIds[i] == SPRITE_NONE)
            sMonSummaryScreen->spriteIds[i] = CreateSprite(&sSpriteTemplate_StatGrades, sGradeIconX[gradeIndex], sGradeIconY[gradeIndex], 0);
