...
#ifndef EXT_CTRL_CODE_TEXT_COLORS
#define EXT_CTRL_CODE_TEXT_COLORS EXT_CTRL_CODE_COLOR_HIGHLIGHT_SHADOW
#endif

static inline bool32 IsBoxMonExcluded(struct BoxPokemon *boxMon)
{
    return FALSE;
}

static inline bool32 CanBoxMonBeSelected(struct BoxPokemon *boxMon)
{
    return TRUE;
}

static inline bool32 TryBoxMonFormChange(struct BoxPokemon *boxMon, u32 method)
{
...
static void SetScrollingBackground(void)
{
    ...
    LZDecompressVram(sSwShWallpapers[wallpaperId].tilemap, (void *)BG_SCREEN_ADDR(31));
...
static void LoadPokeStorageMenuGfx(void)
{
    ...
    LZDecompressWram(sSwShStorage_BG1_Tilemap, sStorage->displayMenuTilemapBuffer);
    SetBgTilemapBuffer(1, sStorage->displayMenuTilemapBuffer);
    ShowBg(1);
    ScheduleBgCopyTilemapToVram(1);

    LZDecompressWram(sSwShStorage_BG2_Tilemap, sStorage->wallpaperBgTilemapBuffer);
...
static void InitPokeStorageBg0(void)
{
    ...
    LZDecompressWram(sMonInfo_Tilemap, sStorage->infoTilemapBuffer);
    sStorage->bg0_Y = 0;
    UpdateMonInfoTilemap();
...
static void StartLoadWallpaperGfx(u8 boxId, s8 direction)
{
    ...
    if (sStorage->wallpaperTiles != NULL)
        LZDecompressWram(src, sStorage->wallpaperTiles);
}
...
static void UpdateWallpaperGfx(u8 boxId, s8 direction)
{
    ...
    LZDecompressWram(sSwShWallpapers[wallpaperId].tilemap, sStorage->wallpaperBgTilemapBuffer);
...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    else if (mode == MODE_BOX)
    {
        ...
        if (sStorage->displayMon.species != SPECIES_NONE)
        {
            ...
            sStorage->displayMon.ability = GetAbilityBySpecies(sStorage->displayMon.species, abilityNum, FALSE);
        }
    }
    else
    {
...
static void MultiMove_SetIconToBg(u8 x, u8 y)
{
    ...
    bool32 isEgg UNUSED = GetCurrentBoxMonData(position, MON_DATA_IS_EGG);
