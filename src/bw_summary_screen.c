...
static void SetMoveTypeIcons(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (move != MOVE_NONE)
        {
            type = P_SHOW_DYNAMIC_TYPES ? CheckDynamicMoveType(&sMonSummaryScreen->currentMon, move, 0, MON_OUTSIDE_BATTLE) : GetMoveType(move);
...
static void SetNewMoveTypeIcon(void)
{
    ...
    else
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
        {
            u32 type = P_SHOW_DYNAMIC_TYPES ? CheckDynamicMoveType(&sMonSummaryScreen->currentMon, move, 0, MON_OUTSIDE_BATTLE) : GetMoveType(move);
